    ...
    for (var i in views) {
			if(checkme) {
				...
				if (views[i].loop_start) {
					views[i].loop_start(-1, myInterval[0], myInterval[1], checkme, page_monitor_pattern, preurl, mySitesList, mySitesChance, preferNewTab);
				}
			} else {
				if (views[i].loop_start) {
					views[i].loop_start(-1, myInterval[0], myInterval[1], null, null, preurl, mySitesList, mySitesChance, preferNewTab);
				}
			}
		}
  } else {
...
function startTimer() {
  if (getId("timerbtn").value == "Start Timer" ) {
	...
	if (!localStorage['pdurl']) {
		var preurl = localStorage['pdurl'];
	} else {
		var preurl = "https://webingest.com/redirect";
	}
		...
		if(waitTime <= 0) {
			...
			for (var i in views) {
				if(checkme) {
					var page_monitor_pattern = getId("pmpattern").value;
					if (views[i].loop_start) {
						views[i].loop_start(waitTime, myInterval[0], myInterval[1], checkme, page_monitor_pattern, preurl, mySitesList, mySitesChance, preferNewTab);
					}
				} else {
					if (views[i].loop_start) {
						views[i].loop_start(waitTime, myInterval[0], myInterval[1], null, null, preurl, mySitesList, mySitesChance, preferNewTab);
					}
				}
			}
...
function restoreOptions() {
	...
	if(localStorage['randomTime'] == 'true') {
