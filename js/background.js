...
if (localStorage['autostart'] == "true" && auto_start_url) {
	...
	loop_start(-1, default_time, 'C', '', '', auto_start_url, '', '',false);
}
...
function decideIfToUseMySites() {
	
	// Load options
	var preurl = getId("pdurlinp").value;

	if (localStorage['preferNewTab'] && localStorage['preferNewTab'] == 'true') {
		var preferNewTab = true;
	}
	else {
		var preferNewTab = false;
	}

	if (localStorage['mySitesList'] && localStorage['mySites'] == 'true') {
		var mySitesList = JSON.parse(localStorage.urls);
	}
	else {
		var mySitesList = [];
	}

	if (localStorage['mySitesChance']) {
		var mySitesChance = localStorage['mySitesChance'];
	}
	else {
		var mySitesChance = 0;
	}
	...
	// Decide if we want to use mySites
	if (mySitesChance == 0 || mySitesList.length == 0) {
		// 1. mySites option disabled or empty, use default url
		return preurl;
	}
	else {
		// Generate a random number 1 to 100, use mySites based on mySitesChance
		var randomNumber = Math.random() * 100;
		
		if (randomNumber < mySitesChance) {
			// 2. use a random link from mySites
			const randomElement = mySitesList[Math.floor(Math.random() * mySitesList.length)];
			return randomElement;
			console.log('randomelement:' + randomElement);
		}
		else {
			// 3. keep using default url
			return preurl;
		}
	}
}
...
function loop_start(waitTime, interval_time, interval_type, checkme, page_monitor_pattern, predefined_url, mySitesList, mySitesChance,preferNewTab) {
...
function setTimerBadgeText(tabId) {
	chrome.browserAction.setBadgeBackgroundColor({color:[0, 22, 184, 167], tabId:tabId});
...
function show_notification(pmpattern, check_content, onclick) {
	...
	var options = {
	  ...
	  iconUrl: "icon/icon-webshuffle.png",
